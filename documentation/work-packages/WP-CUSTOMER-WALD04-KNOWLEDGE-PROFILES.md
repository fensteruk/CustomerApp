...
G09's named Fenster data owner remains pending only for unattended production disposal.
No automatic deletion, source-site binding runtime, WALD05, UI, import commit or deployment.
Answer, save draft and activation remain separate; same authorised Office actor may perform them.
G05 history retention is 24 months after retirement/revocation, extended by active dependencies
and holds. Processing retention metadata is in scope; processing/upload orchestration is not.
The governance table's previous approval requirements are satisfied by DEC-046, except the
named-owner/disposal and final-import audit decisions explicitly deferred to WALD05/production.
