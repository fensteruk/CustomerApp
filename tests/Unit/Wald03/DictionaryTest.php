...
it('treats a genuinely blank call type as a resolved no-visit fact', function () {
    $result = (new Dictionary)->callType(' ');

    expect($result->classification)->toBe(C::Confirmed)
        ->and($result->resolution)->toBe(R::Resolved)
        ->and($result->lookupValue)->toBeNull()
        ->and($result->value)->toBeNull()
        ->and($result->reasons)->toBe(['BLANK_CALL_TYPE_NO_VISIT']);
});
...
it('uses exact field treatments and never invents Portal dates', function ($header, $role, $classification) {
    ...
    ['Customer Number', 'source_customer_code', C::Confirmed], ['Customer Numbers', null, C::Unknown],
...
it('has canonical stable immutable versioned identity', function () {
    ...
    expect($identity->version)->toBe('customerapp.source-dictionary.v10')
        // v10 is frozen: a definition edit must introduce a new version, not update this pair.
...
it('excludes only the approved exact source CustomerCodes', function (): void {
    foreach (['XXTrade', ' xxtest ', '83'] as $code) {
        expect(Dictionary::excludesCustomerCode($code))->toBeTrue();
    }
    foreach (['XXTrade2', 'XXTESTING', '083', 'FNA2664', 'FNA2699'] as $code) {
        expect(Dictionary::excludesCustomerCode($code))->toBeFalse();
    }
});
...
it('requires a version and fingerprint change for labels meanings and mappings', function ($field, $value) {
    ...
    $new = DictionaryIdentity::fromDefinition('customerapp.source-dictionary.v11', $definition, $old);
