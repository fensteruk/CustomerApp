...
class SourceProjectionImportService
{
    ...
    public function import(string $sourceName, iterable $records, ?string $sourceVersion = null): SourceImportRun
    {
        ...
        $records = Collection::make($records)->values();
    ...
    private function applyRecord(SourceImportRun $run, string $sourceName, SourceRecord $record): array
    {
        ...
        if ($existing !== null && ((int) $existing->projectedPlot->site_id !== (int) $site->id || $existing->projectedPlot->plot_reference !== $record->plotReference)) {
        ...
        $service->fill(['source_call_number' => $record->callNumber, 'source_call_type' => $record->callType, 'source_job_stage' => $record->jobStage, 'source_completed_at' => $record->completedDate, 'source_completion_observed_at' => $isComplete ? now() : null, 'source_updated_at' => $record->sourceUpdatedAt, 'last_observed_at' => now(), 'source_present' => true, 'source_missing_since' => null, 'last_source_import_run_id' => $run->id]);
        $outcome = $service->exists && $service->isDirty() ? 'records_updated' : ($service->exists ? 'records_unchanged' : 'records_created');
        $service->save();
        ...
        $plot->update(['source_updated_at' => $record->sourceUpdatedAt, 'synchronised_at' => now()]);
