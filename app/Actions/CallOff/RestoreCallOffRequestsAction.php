...
class RestoreCallOffRequestsAction
{
    ...
    public function handle(User $actor, iterable $requests, ?string $customerResponse = null): CallOffBatchOperation
    {
        return DB::transaction(function () use ($actor, $requests, $customerResponse): CallOffBatchOperation {
            ...
            foreach ($lockedRequests as $request) {
                $this->eligibility->ensureCanRestore($actor, $request);
            }

            $operation = $this->createOperation($actor, $batchId, CallOffOperationType::Restore);
            ...
            foreach ($lockedRequests as $request) {
                ...
                $this->recordHistory->handle($request, $actor, CallOffHistoryEventType::Restored, $status, $status, $beforeState, $afterState, $customerResponse, null, $operation);
            }
