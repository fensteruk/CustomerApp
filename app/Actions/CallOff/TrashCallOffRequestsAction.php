...
class TrashCallOffRequestsAction
{
    ...
    public function handle(User $actor, iterable $requests, ?string $customerResponse = null): CallOffBatchOperation
    {
        return DB::transaction(function () use ($actor, $requests, $customerResponse): CallOffBatchOperation {
            ...
            foreach ($lockedRequests as $request) {
                $this->eligibility->ensureCanTrash($actor, $request);
            }

            $operation = $this->createOperation($actor, $batchId, CallOffOperationType::Trash);
            ...
            foreach ($lockedRequests as $request) {
                ...
                $this->recordHistory->handle($request, $actor, CallOffHistoryEventType::Trashed, $status, $status, $beforeState, $afterState, $customerResponse, null, $operation);
            }
