                            ...
                            <tbody class="divide-y divide-slate-200">
                                @foreach ($plots as $overview)
                                    <tr @class(['bg-indigo-50' => $overview->overallStatus === App\Enums\PlotOverallStatus::FullyCompleted, 'bg-white' => $overview->overallStatus !== App\Enums\PlotOverallStatus::FullyCompleted])>
                                        <th scope="row" class="px-3 py-3 align-top text-sm font-extrabold text-slate-950"><label class="flex min-h-10 items-center gap-2"><input type="checkbox" name="plots[]" value="{{ $overview->plot->uuid }}" class="h-5 w-5 shrink-0 rounded border-slate-300 text-sky-700 focus:ring-sky-700" x-model="selected" @disabled($overview->overallStatus === App\Enums\PlotOverallStatus::FullyCompleted)><span class="min-w-0 [overflow-wrap:anywhere]">{{ $overview->plot->plot_reference }}</span></label></th>
                ...
                <form method="POST" action="{{ route('portal.call-offs.dashboard-selection') }}" class="mt-4 space-y-3 xl:hidden" x-data="{ selected: [] }">
                    ...
                    @foreach ($plots as $overview)
                        <article @class(['rounded-xl border p-4 shadow-sm' => true, 'border-indigo-300 bg-indigo-50' => $overview->overallStatus === App\Enums\PlotOverallStatus::FullyCompleted, 'border-slate-200 bg-white' => $overview->overallStatus !== App\Enums\PlotOverallStatus::FullyCompleted]) aria-labelledby="plot-{{ $overview->plot->uuid }}">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div class="flex min-w-0 items-start gap-3">@if ($overview->overallStatus !== App\Enums\PlotOverallStatus::FullyCompleted)<input type="checkbox" name="plots[]" value="{{ $overview->plot->uuid }}" class="mt-1 h-5 w-5 shrink-0 rounded border-slate-300 text-sky-700 focus:ring-sky-700" x-model="selected" aria-label="Select {{ $overview->plot->plot_reference }} for a call off">@endif<h3 id="plot-{{ $overview->plot->uuid }}" class="min-w-0 text-xl font-bold text-slate-950 [overflow-wrap:anywhere]">{{ $overview->plot->plot_reference }}</h3></div><x-plot-overall-status :status="$overview->overallStatus" /></div>
