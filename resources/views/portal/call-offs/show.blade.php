    ...
    <section class="mx-auto max-w-5xl px-4 py-7 sm:px-6 lg:px-8" aria-labelledby="page-title">
        ...
        <div class="mt-7 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <span class="status status-{{ $isCompleted ? 'slate' : $callOffRequest->status->tone() }}">
                {{ $isCompleted ? 'Completed' : ($callOffRequest->isLegacyDateAgreed() ? 'Date Agreed' : $callOffRequest->status->label()) }}
            </span>
        </div>
        ...
        <section class="mt-8 rounded-xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="request-summary-heading">
            ...
            <dl class="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div><dt class="font-semibold text-slate-600">Submitted</dt><dd class="mt-1 text-base font-bold text-slate-950">{{ $callOffRequest->batch->submitted_at?->format('j M Y, H:i') ?? 'Not available' }}</dd></div>
                @if ($agreedDate !== null)
        ...
        @if (! $isCompleted && $callOffRequest->status === App\Enums\CallOffRequestStatus::AwaitingFenster)
        ...
        @elseif (! $isCompleted && $callOffRequest->status === App\Enums\CallOffRequestStatus::AwaitingSiteUser && $currentProposal)
            <section class="mt-6 rounded-xl border border-sky-300 bg-sky-50 p-5" aria-labelledby="alternative-heading">
                ...
                <dl class="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                    <div><dt class="font-semibold text-sky-900">Original requested date</dt><dd class="mt-1 text-lg font-bold text-sky-950">{{ $requestDate?->format('j M Y') ?? 'Not available' }}</dd></div>
            ...
            <ol class="mt-5 space-y-4" aria-label="Customer-facing request history">
                @foreach ($callOffRequest->histories as $history)
                    <li class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><h3 class="font-bold text-slate-950">{{ $history->event_type->label() }}</h3><time class="text-sm text-slate-600" datetime="{{ $history->performed_at?->toIso8601String() }}">{{ $history->performed_at?->format('j M Y, H:i') ?? 'Time not available' }}</time></div>
                        <p class="mt-2 text-sm text-slate-700">{{ $history->performedBy?->name ?? 'Fenster Customer Portal' }}@if ($history->performedBy?->portalRole), {{ $history->performedBy->portalRole->name }}@endif</p>
                        ...
                        @if ($history->event_type === App\Enums\CallOffHistoryEventType::DateAgreed && $agreedDate)<p class="mt-3 text-sm font-bold text-emerald-800">Date Agreed — {{ $agreedDate->format('j M Y') }}</p>@endif
                    </li>
                @endforeach
