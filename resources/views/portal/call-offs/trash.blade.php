    ...
    <section class="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8" aria-labelledby="page-title">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <h1 id="page-title" class="page-title">Trash</h1>
                <p class="page-intro">Rejected and withdrawn call-offs can be restored for seven days. Expired items remain in audit history but are not shown here.</p>
            </div>
        ...
        <form method="POST" action="{{ route('portal.call-offs.lifecycle.confirm') }}" class="mt-8 space-y-3">
            @csrf
            @forelse ($trashedRequests as $callOffRequest)
                <article class="request-card">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div><h2 class="text-lg font-bold text-slate-900">{{ $callOffRequest->projectedPlot->plot_reference }}</h2><p class="mt-1 text-sm text-slate-600">{{ $callOffRequest->batch->service_identifier->label() }} · requested {{ $callOffRequest->batch->requested_date->format('j M Y') }}</p></div>
                    ...
                    <label class="mt-5 flex min-h-12 items-center gap-3 rounded-lg bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800"><input type="checkbox" name="requests[]" value="{{ $callOffRequest->uuid }}" class="h-5 w-5 rounded border-slate-300 text-sky-700 focus:ring-sky-700"> Select this call-off to restore</label>
                </article>
            @empty
            ...
            @if ($trashedRequests->isNotEmpty())
                <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p class="text-sm text-slate-700">Select one or more call-offs from the same submission batch. All selected items must still be restorable for the action to succeed.</p><button type="submit" name="operation" value="restore" class="primary-button mt-4">Restore selected</button></div>
            @endif
        </form>
    </section>
</x-layouts.portal>
